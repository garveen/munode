    ...
    pub(super) async fn handle_register(
        ...
        if let Some(hmac_secret) = &self.state.config.registry.hmac_secret {
            if params.challenge_response.is_none() {
                ...
                return Ok(self.make_response_packet(request_id, "edge.register", |response| {
                ...
                (&params.challenge, &params.challenge_response)
            {
                ...
                if *response != expected {
                    return Ok(self.make_response_packet(request_id, "edge.register", |packet| {
        ...
        if !stale_sessions.is_empty() {
            if is_additional_pool_slot {
                ...
                for session in &stale_sessions {
                    ...
                    self.broadcast_notification_excluding("hub.userRemoveBroadcast", reconnecting_edge_id, |notification| {
                ...
                info!(
                    ...
                    stale_sessions.len(), params.server_id
                );
            }
        }
    ...
    pub(super) async fn handle_authenticate_user(
        ...
        if let Some(re) = &self.username_regex {
            if !re.is_match(username) {
                ...
                return Ok(self.make_response_packet(request_id, "edge.authenticateUser", |response| {
...

        {
            ...
            if !new_cert.is_empty() {
                ...
                if let Some(existing) = all_sessions.iter().find(|session| session.username.eq_ignore_ascii_case(username)) {
...

        {
            ...
            if max_users > 0 {
                ...
                if current >= max_users as usize {
                    warn!("Rejecting user '{}': server at capacity ({}/{})", username, current, max_users);
                    return Ok(self.make_response_packet(request_id, "edge.authenticateUser", |response| {
                        response.edge_authenticate_user = Some(EdgeAuthenticateUserResult {
                            success: false,
                            user_id: None,
        ...
        if self.state.auth_service.is_connected().await {
            ...
            let ext_request = ExtAuthRequest {
                ...
                server_id: params.server_id,
                ip_address: client_info.map(|client| client.ip_address.clone()).unwrap_or_default(),
                ip_version: client_info.map(|client| client.ip_version.clone()).unwrap_or_default(),
                release: client_info.map(|client| client.release.clone()).unwrap_or_default(),
                version: client_info.and_then(|client| client.version),
                os: client_info.map(|client| client.os.clone()).unwrap_or_default(),
                os_version: client_info.map(|client| client.os_version.clone()).unwrap_or_default(),
            ...
            match self.state.auth_service.authenticate(ext_request).await {
                Some(resp) => {
                    ...
                    if user_id > 0 {
                        ...
                        if let Err(error) = self.state.user_store.upsert_ext_user(user_id, &auth_username_owned).await {
                    ...
                    let channel_id = if let Some(channel_id) = resp.channel_id {
                        ...
                        if last_channel > 0 && self.state.channel_store.get_channel(last_channel).await.is_some() {
                            last_channel
                        } else {
                    ...
                    let session_info = SessionInfo {
                        ...
                        cert_hash: params.client_info.as_ref().and_then(|client| client.certificate_hash.clone()).unwrap_or_default(),
                    ...
                    let cert_hash = params.client_info.as_ref().and_then(|client| client.certificate_hash.clone());
                ...
                None => {
                    ...
                    warn!("Auth service request timed out for user '{}'; falling back to local auth", username);
                }
            }
        ...
        if let Some(lua_engine) = lua_engine_guard.as_ref() {
            ...
            let lua_req = LuaAuthRequest {
                ...
                tokens: params.tokens.clone(),
                server_id: params.server_id,
                ip: client_info.map(|client| client.ip_address.clone()).unwrap_or_default(),
                ip_version: client_info.map(|client| client.ip_version.clone()).unwrap_or_default(),
                release: client_info.map(|client| client.release.clone()).unwrap_or_default(),
                version: client_info.and_then(|client| client.version),
                os: client_info.map(|client| client.os.clone()).unwrap_or_default(),
                osversion: client_info.map(|client| client.os_version.clone()).unwrap_or_default(),
                certificate_hash: client_info.and_then(|client| client.certificate_hash.clone()),
            };
            ...
            match engine.authenticate(lua_req).await {
                ...
                Err(error) => {
                    warn!("Lua auth error for '{}': {:#}; falling back to next auth method", username, error);
                    if config.auth.require_auth_service {
                        return Ok(self.make_response_packet(request_id, "edge.authenticateUser", |response| {
                            response.edge_authenticate_user = Some(EdgeAuthenticateUserResult {
                                success: false,
                                ...
                                display_name: None,
                                groups: vec![],
                                reason: Some(format!("Authentication script error: {error}")),
                                reject_type: Some(8),
                                channel_id: None,
                                mute: None,
                                deaf: None,
        ...
        if let Some(ref http_url) = config.auth.http_url.clone() {
            ...
            match http_result {
                ...
                Err(error) => {
                    warn!("HTTP auth error for user '{}': {}; falling back to local auth", username, error);
                    if config.auth.require_auth_service {
                        return Ok(self.make_response_packet(request_id, "edge.authenticateUser", |response| {
                            response.edge_authenticate_user = Some(EdgeAuthenticateUserResult {
                                success: false,
                                ...
                                display_name: None,
                                groups: vec![],
                                reason: Some(format!("Authentication service error: {}", error)),
                                reject_type: Some(8),
                                channel_id: None,
                                mute: None,
                                deaf: None,
        ...
        let password_ok = if let Some(user) = db_user.as_ref() {
            ...
            let user_id = user.id;
            let password_hash = tokio::task::spawn_blocking(move || database.get_user_password_hash(user_id))
                .await
        ...
        let channel_id = if restored_channel_id > 0
            && self.state.channel_store.get_channel(restored_channel_id).await.is_none()
        {
            config.auth.default_channel
        } else {
            restored_channel_id
        ...
        if self.state.geoip.is_available() && self.state.config.geoip.log_location {
            let ip_str = params.client_info.as_ref().map(|client| client.ip_address.as_str()).unwrap_or("");
        ...
        Ok(self.make_response_packet(request_id, "edge.authenticateUser", |response| {
            response.edge_authenticate_user = Some(EdgeAuthenticateUserResult {
                success: true,
                user_id: Some(user_id),
