...
impl RpcHandler {
    ...
    pub(super) async fn handle_report_session(
        ...
        let session = params.session.as_ref()
        ...
        let already_known = self.state.session_manager.get_session(session.session_id).await
        ...
        Ok(self.make_response_packet(request_id, "edge.reportSession", |response| {
            response.edge_report_session = Some(EdgeReportSessionResult { success: true, error: None });
        }))
    }
    ...
    pub(super) async fn handle_user_left_rpc(
        ...
        let params = request.edge_user_left.as_ref()
        ...
        if let Some(removed) = self.state.session_manager.remove_session(session_id).await {
            ...
            self.broadcast_notification_excluding("hub.userRemoveBroadcast", edge_server_id, |notification| {
        ...
        Ok(self.make_response_packet(request_id, "edge.userLeft", |response| {
            response.edge_user_left = Some(EdgeUserLeftResult { success: true, error: None });
        }))
    }
    ...
    pub(super) async fn handle_user_moved_rpc(
        ...
        let params = request.edge_user_moved.as_ref()
            .context("Missing edge_user_moved params")?;
        if params.session_id == 0 {
            return Ok(self.make_response_packet(request_id, "edge.userMoved", |response| {
        ...
        if let Some(target_channel) = self.state.channel_store.get_channel(params.channel_id).await {
        ...
        Ok(self.make_response_packet(request_id, "edge.userMoved", |response| {
            response.edge_user_moved = Some(EdgeUserMovedResult { success: true, error: None });
        }))
    }
    ...
    pub(super) async fn handle_user_state_changed_rpc(
        ...
        let params = request.edge_user_state_changed.as_ref()
            .context("Missing edge_user_state_changed params")?;
        if params.session_id == 0 {
            return Ok(self.make_response_packet(request_id, "edge.userStateChanged", |response| {
        ...
        if let Some(mut session) = sessions.get_session(params.session_id).await {
            if let Some(value) = params.self_mute { session.self_mute = value; }
            if let Some(value) = params.self_deaf { session.self_deaf = value; }
            if let Some(value) = params.mute { session.mute = value; }
            if let Some(value) = params.deaf { session.deaf = value; }
            if let Some(value) = params.suppress { session.suppress = value; }
            if let Some(value) = params.priority_speaker { session.priority_speaker = value; }
            if let Some(value) = params.recording { session.recording = value; }
            ...
            for &channel_id in &params.listening_channel_add {
                if session.listening_channels.contains(&channel_id) || validated_adds.contains(&channel_id) {
                    continue;
                }
                if per_user_limit > 0
                ...
                if per_channel_limit > 0 {
                    let channel_count = all_sessions_snapshot
                        .as_ref()
                        .map(|sessions| sessions.iter().filter(|entry| entry.listening_channels.contains(&channel_id)).count())
                        .unwrap_or(0);
        ...
        Ok(self.make_response_packet(request_id, "edge.userStateChanged", |response| {
            response.edge_user_state_changed = Some(EdgeUserStateChangedResult { success: true, error: None });
        }))
    }
    ...
    pub(super) async fn handle_channel_state_rpc(
        ...
        let params = request.edge_channel_state.as_ref()
        ...
        Ok(self.make_response_packet(request_id, "edge.channelState", |response| {
            response.edge_channel_state = Some(EdgeChannelStateResult { success: true, error: None });
        }))
    }
    ...
    pub(super) async fn handle_channel_remove_rpc(
        ...
        let params = request.edge_channel_remove.as_ref()
        ...
        Ok(self.make_response_packet(request_id, "edge.channelRemove", |response| {
            response.edge_channel_remove = Some(EdgeChannelRemoveResult { success: true, error: None });
        }))
    }
    ...
    pub(super) async fn handle_user_remove_rpc(
        ...
        let params = request.edge_user_remove.as_ref()
            .context("Missing edge_user_remove params")?;
        let target_session = params.target_session;
        if target_session == 0 {
            return Ok(self.make_response_packet(request_id, "edge.userRemove", |response| {
        ...
        let required_permission = if params.ban { permission::BAN } else { permission::KICK };
        ...
        Ok(self.make_response_packet(request_id, "edge.userRemove", |response| {
            response.edge_user_remove = Some(EdgeUserRemoveResult { success: true, error: None });
        }))
    }
}