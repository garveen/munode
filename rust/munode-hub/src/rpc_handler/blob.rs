...
impl RpcHandler {
    ...
    pub(super) async fn handle_blob_put(
        ...
        let params = request
            .blob_put
            .as_ref()
            .context("Missing blob_put params")?;
    ...
    pub(super) async fn handle_blob_get(
        ...
        let params = request
            .blob_get
            .as_ref()
            .context("Missing blob_get params")?;
    ...
    pub(super) async fn handle_blob_get_user_texture(
        ...
        let params = request
            .blob_get_user_texture
            .as_ref()
            .context("Missing blob_get_user_texture params")?;
        let hash_opt = self
            .state
            .user_store
            .get_blob_hash(params.user_id, "texture")
            .await?;
        match hash_opt {
            Some(hash) => match self.state.blob_store.get_async(hash.clone()).await {
                Ok(Some(data)) => {
                    Ok(
                        self.make_response_packet(request_id, "blob.getUserTexture", |r| {
                ...
                Ok(None) => Ok(
                    self.make_response_packet(request_id, "blob.getUserTexture", |r| {
                        r.blob_get_user_texture = Some(BlobGetUserTextureResult {
                            ...
                            data: None,
                            hash: None,
                            error: Some("Blob data not found".into()),
                        });
                    }),
                ),
                Err(e) => Ok(
    ...
    pub(super) async fn handle_blob_get_user_comment(
        ...
        let params = request
            .blob_get_user_comment
            .as_ref()
            .context("Missing blob_get_user_comment params")?;
        let hash_opt = self
            .state
            .user_store
            .get_blob_hash(params.user_id, "comment")
            .await?;
        match hash_opt {
            Some(hash) => match self.state.blob_store.get_async(hash.clone()).await {
                Ok(Some(data)) => {
                    Ok(
                        self.make_response_packet(request_id, "blob.getUserComment", |r| {
    ...
    pub(super) async fn handle_blob_set_user_texture(
        ...
        let params = request
            ...
            .as_ref()
            .context("Missing blob_set_user_texture params")?;
        if params.data.len() > Self::MAX_BLOB_BYTES {
            return Ok(
                self.make_response_packet(request_id, "blob.setUserTexture", |r| {
        ...
        match self.state.blob_store.put_async(params.data.clone()).await {
            Ok(hash) => {
                match self
                    .state
                    .user_store
                    .set_blob_hash(params.user_id, "texture", &hash)
                    .await
                {
                    Ok(()) => {
                        Ok(
    ...
    pub(super) async fn handle_blob_set_user_comment(
        ...
        let params = request
            ...
            .as_ref()
            .context("Missing blob_set_user_comment params")?;
        if params.data.len() > Self::MAX_BLOB_BYTES {
            return Ok(
                self.make_response_packet(request_id, "blob.setUserComment", |r| {
        ...
        match self.state.blob_store.put_async(params.data.clone()).await {
            Ok(hash) => {
                match self
                    .state
                    .user_store
                    .set_blob_hash(params.user_id, "comment", &hash)
                    .await
                {
                    Ok(()) => {
                        Ok(
