...
6. **统计数据说明**：本表按 2026-03-12 代码状态更新，相比 2026-03-10 版本新增约 40 个 ✅ 项；新增集成测试：`lua-auth.test.ts`（Lua 认证，9 用例）、`voice-routing-strategy.test.ts`（三种 connection_strategy，9 用例）、`log-format.test.ts`（JSON 日志格式，5 用例）、`hub-connection-pool.test.ts`（pool_size=1/3，7 用例）、`diagnose.test.ts`（Hub/Edge 诊断工具，13 用例）；新增功能：`diagnose` 子命令（Hub/Edge）、每 Edge 标签化 Prometheus 指标
