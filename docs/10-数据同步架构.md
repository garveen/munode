...
> **⚠️ 实施建议：** 本文档描述了完整的 Edge 全状态同步架构。但对于预期用户规模 < 1000 的部署，**不建议实施本方案**。Hub 集中计算模式已足够满足性能需求，且大幅降低系统复杂度。本文档可作为未来扩展的参考，或用于超大规模部署场景（5000+ 并发用户）。
>
> **当前推荐架构：** Hub 集中计算 + Edge 作为连接代理（详见文档末尾的"适用场景分析"）
...
class HubPermissionChecker {
  ...
  async hasPermission(...): Promise<boolean> {
    const key = `${userId}:${channelId}:${permission}`;
    const cached = this.cache.get(key);
    if (cached !== undefined) return cached;
    
    const result = await this.calculatePermission(...);
...
| 架构模式 | Hub CPU | Edge CPU | 平均延迟 | 复杂度 | 维护成本 |
|---------|---------|---------|---------|--------|----------|
| Hub 集中计算 | 30-40% | 10-15% | 20-30ms | 低 | 低 |
| Edge 全状态计算 | 10-15% | 40-50% | 5-10ms | 高 | 高 |
| 单体架构 | 40-50% | - | 1-5ms | 最低 | 最低 |

**结论：** 对于 < 1000 用户，延迟差异（20ms vs 5ms）用户无法感知，不值得增加系统复杂度。
...
**对于本项目（< 1000 用户）：明确不需要实施 Edge 全状态计算。**
